...
#2. Bricando com o exemplo das pizzas#########
...
#2c. Traçando um modelo de regressao######
...
meus_desenhos <- dados_pizzaria %>% mutate(chute_media = mean(dados_pizzaria$pizza),chute_modelo = reg_simples$fitted.values)
...
#2d.. Através das métricas de qualidade de ajuste######
...
Bodyfat <- read_csv("D:/OneDrive/Pessoal/ASN.Rocks/Regressão Linear/dados/Bodyfat.csv")
# tirando uma variavel que nao sera usada (contexto negocio)
Bodyfat <- Bodyfat %>%
  select(-Density)
View(Bodyfat)

#3a. Conhecendo minha variável y#####################################################

# Faça uma breve descritiva da variavel y
...
#FAÇA VOCÊ - Explore a variavel Wrist###########################
# a variavel explicativa (Wrist)


#FAÇA VOCÊ - Visualize a relação entre X e Y###########################
...
#FAÇA VOCÊ - Crie a tentativa média e a modelo###########################
...
#4a... Através das métricas de qualidade de ajuste######
#FAÇA VOCÊ - Calcule MSE e RMSE desses dois chutes###########################
...
reg_gordura_full <- lm(bodyfat ~ Neck+Abdomen+Forearm+Wrist, data=Bodyfat)
summary(reg_gordura_full)

#SIMULação 2

reg_gordura_full <- lm(bodyfat ~ Neck+Abdomen+Wrist, data=Bodyfat)
summary(reg_gordura_full)
...
reg_gordura_full <- lm(bodyfat ~ Age+
                         Weight+
                         Height+
                         Neck+
                         Abdomen+
...
# Tentamos retirar a Idade devido a baixa correlação com Y, mas piorou o resultado do modelo
...
#' na volta discutiremos!


#6. Criando o modelo com seleção de variável###############################
...
#7. Pausa para discussão sobre os parâmetros###############################

#7a. Justificando porque não é bom tirar b0 do modelo###############################
...
#7b. Comparar efeitos e variações###############################
...
# observado a diferença do modelo completo para o modelo stepwise por p-valor
...
#9. Pausa para discussão sobre MULTICOLINEARIDADE###############################

#9a. Primeiro vamos entender o malefício###############################

# para falarmos sobre isso trouxe uma outra base de dados
# onde o objetivo eh prever o tempo de viagem
# dado o número de milhas percorridas e galoes gastos
galoes <- read_excel("D:/OneDrive/Pessoal/ASN.Rocks/Regressão Linear/dados/Galoes.xlsx")
galoes
...
# Bom, sem dúvida significa não colocar variaveis (X´s)
# altamente correlacionadas entre si e multivariadamente também
...
#9c. Entendendo a situação do modelo Bodyfat stepwise criterio AIC###############################

# repare que existem muitas variaveis X´s com forte
# relacoes com outras variaveis X´s
chart.Correlation((Bodyfat[2:15]), histogram = TRUE)
...
#10. FAÇA EM GRUPO - Construa o modelo lindao estatisticamente###############################
